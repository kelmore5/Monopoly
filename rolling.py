...
def position(x, y):
    ...
    return resolution
def status(self, player, player1, player2, player3):
    if player is player1:
        title = "Player 1's Turn"
    elif player is player2:
        title = "Player 2's Turn"
    elif player is player3:
        title = "Player 3's Turn"
...
def refresh(self):
    ...
    self.dog.Refresh()
    return self
def landing(self, player, player2, panelstart, player3):
    ...
    if price == "CC":
        ...
        if panelstart[player[1]] == [68, 630]:
            ...
            refresh(self)
        elif panelstart[player[1]] == [94, 630]:
            self.dog.Destroy()
        ...
        dlg.Destroy()
        return
    elif space in player3:
        rent = rents[player[1]]
        dlg = GMD.GenericMessageDialog(self, "You landed on %s.\n%s is owned by Player 3. You have $%s.\nYou have to pay $%s to the other Player.""" %(space, space, player[0], rent), "Property Owned", wx.OK)
        result = dlg.ShowModal()
        player[0] -= rent
        player3[0] += rent
        dialog.Destroy()
