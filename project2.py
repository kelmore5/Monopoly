...
class MainWindow(wx.Frame):
    ...
    def movement(self, player):
        if not "money" in dir(self) or not "racecar" in dir(self):
            ...
            rolling2.refresh(self)
            self.movement(player)
        if player is player1:
            rolling2.status(self, player, player1, player2)
            self.money.Destroy()
            rolling2.rolling(self, player)
            ...
            panel = rolling2.onspace(self, panelstart[player[1]])
            MoneyBag = wx.BitmapFromImage(wx.Image("Money.jpg", wx.BITMAP_TYPE_ANY), depth=1)
            self.money = wx.StaticBitmap(panel, -1, MoneyBag)
            rolling2.refresh(self)
            rolling2.landing(self, player, player2, panelstart)
            rolling2.status(self, player, player1, player2)
            rolling2.refresh(self)
            player = player2
        if player is player2:
            ...
            self.racecar = wx.StaticBitmap(panel, -1, RaceCar)
            rolling2.refresh(self)
            rolling2.landing(self, player, player1, panelstart)
            rolling2.status(self, player, player1, player2)
            rolling2.refresh(self)
            player = player1
            self.movement(player)
